...
export async function up(db: Kysely<any>): Promise<void> {
	...
	await db.schema
		...
		.addColumn('commission_locked', 'boolean', (col) => col.notNull().defaultTo(false))
		.addColumn('created_at', 'timestamptz', (col) => col.notNull().defaultTo(sql`now()`))
		.addColumn('updated_at', 'timestamptz', (col) => col.notNull().defaultTo(sql`now()`))
		.execute()
...
export async function down(db: Kysely<any>): Promise<void> {
	...
	await db.schema.dropIndex('lead_activities_buyer_lead_idx').ifExists().execute()
	await db.schema.dropIndex('lead_activities_seller_lead_idx').ifExists().execute()
